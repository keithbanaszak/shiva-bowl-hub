...
export default function PlayersPage() {
  ...
  const startCols: ColumnSpec[] = [
    ...
    },
    {
      ...
      descFirst: false,
      headerTitle:
        "The manager who started him most — and their record in those weeks",
    },
  ];
  ...
  const startRows: TableRow[] = starts.map((r) => {
    ...
    return {
      ...
      cells: {
        player: (
          <PlayerCell playerId={r.playerId} size={22} sub={pteam(r.playerId)} />
        ),
    ...
    <div>
      ...
      <div className="mb-6">
        <Note title="What counts here">
          ...
          qualify). <strong>Started most by</strong> now also shows that
          manager’s own record in the weeks they started him.{" "}
          <strong>Bench points</strong> are points a player scored while sitting
          on someone’s bench — pure hindsight pain. Click any player for their
          full <strong>league legacy</strong>. Every column sorts.
        </Note>
      </div>
      ...
      <div className="mb-8">
        <DataTable
          ...
          columns={startCols}
          rank
          initialSort={{ key: "winpct", dir: "desc" }}
        />
      </div>
      ...
      <div className="mb-3">
        <Note>
          <span className="text-[var(--muted)]">
            ...
            <strong>% above</strong> is the honest comparison: a +5 lift means
            far more for a 6-point player than a 25-point one.
          </span>
        </Note>
      </div>
      <DataTable
