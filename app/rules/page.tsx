...
const STATUS: Record<LeagueRule["status"], { label: string; cls: string }> = {
  ...
  proposed: {
    label: "On the ballot",
    cls: "bg-[var(--gold-soft)] text-[var(--gold)]",
  },
  ...
  return (
    ...
    >
      <div className="mb-1 flex flex-wrap items-center gap-2">
        ...
        <span
          className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${s.cls}`}
        >
          {s.label}
        </span>
        {r.status === "proposed" && r.voteCloses && (
          <span className="text-[11px] text-[var(--muted)]">
            vote closes {r.voteCloses}
          </span>
        )}
      </div>
      {r.detail && (
...
export default function RulesPage() {
  ...
  const active = leagueConfigData.rules.filter(
    (r) => r.status === "active",
  ).length;
    ...
    <div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Stat
          ...
          value={
            fetched
              ? new Date(fetched).toLocaleDateString("en-US", {
                  month: "short",
                  day: "numeric",
                })
              : "—"
      ...
      {proposed.length > 0 && (
        <div className="mb-8">
          <Note
            title={`${proposed.length} rule${proposed.length === 1 ? "" : "s"} currently being voted on`}
          >
            Anything marked <strong>on the ballot</strong> is a proposal, not a
            rule yet. The draft-order proposal below is what the projected order
            on the{" "}
            <a
              href="/playoffs"
            ...
            page is calculated from, so you can see exactly what it would
            produce this year before voting.
          </Note>
        </div>
      )}
      ...
      <div className="mt-10">
        <Note title="How to edit this page">
          {linked ? (
            <>
              These come from the league Google Sheet. Edit a row there, then
              run{" "}
              <code className="rounded bg-[var(--chip)] px-1">
                npm run config:pull
              </code>{" "}
              (or just wait for the weekly refresh) and the page updates.
              Columns: <strong>category</strong>, <strong>rule</strong>,{" "}
              <strong>detail</strong>, <strong>status</strong> (active /
              proposed / retired), <strong>vote_closes</strong>,{" "}
              <strong>sort_order</strong>, <strong>pinned</strong>.
            </>
          ) : (
            <>
              ...
              . To edit them in a spreadsheet instead: create a Google Sheet
              with a <strong>rules</strong> tab (columns{" "}
              <strong>
                category, rule, detail, status, vote_closes, sort_order, pinned
              </strong>
              ) and a <strong>managers</strong> tab (columns{" "}
              <strong>
                user_id, real_name, nickname, joined, favorite_team, bio
              </strong>
              ), publish it to the web, then put its id and tab gids into{" "}
              <code className="rounded bg-[var(--chip)] px-1">
                league.config.ts
              </code>
              .
