...
import { Card, PageHeader, SectionTitle, Badge, Note, Tag } from "@/components/ui";
...
function Bar({ value, tone = "accent" }: { value: number; tone?: "accent" | "accent2" | "muted" }) {
  const color =
    tone === "accent" ? "bg-emerald-400" : tone === "accent2" ? "bg-cyan-400" : "bg-white/40";
  return (
...
function AxisSlider({ axis }: { axis: number }) {
  ...
  const tone = axis > 20 ? "bg-emerald-400" : axis < -20 ? "bg-cyan-400" : "bg-amber-300";
  return (
    <div className="relative h-2 w-full rounded-full bg-gradient-to-r from-cyan-500/25 via-white/10 to-emerald-500/25">
      <div className="absolute top-1/2 h-1 w-px -translate-y-1/2 bg-white/30" style={{ left: "50%" }} />
      <div
    ...
    <Card>
      <div className="mb-3 flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <Link href={`/managers/${t.userId}`} className="block truncate font-display font-semibold hover:text-emerald-300">
            {label(t.userId)}
          </Link>
          ...
          <div key={pos}>
            <div
              className={`mb-1 flex justify-between text-[10px] ${
                pos === t.weakestPos ? "text-red-300" : "text-[var(--muted)]"
              }`}
            >
              <span>{pos}</span>
              <span className="font-mono tabular-nums">{Math.round(t.posStrength[pos])}</span>
            </div>
      ...
      {/* top assets */}
      <div className="mb-3">
        <div className="mb-1 text-[10px] uppercase tracking-wider text-[var(--muted)]">Top assets</div>
        <div className="flex flex-wrap gap-1.5">
          {t.topAssets.map((p) => (
            <span key={p.playerId} className="flex items-center gap-1.5 rounded-full bg-white/5 px-2 py-1 text-xs">
              <PlayerAvatar playerId={p.playerId} size={18} />
              <span className="truncate">{p.name}</span>
              <span className="font-mono text-[10px] text-[var(--muted)]">{p.score}</span>
            ...
            t.futurePicks.map((pk, i) => (
              <span
                ...
                className={`rounded px-1.5 py-0.5 text-[10px] ${
                  pk.fromUserId ? "bg-cyan-400/10 text-cyan-300" : "bg-white/5 text-[var(--muted)]"
                }`}
                title={pk.fromUserId ? `via ${label(pk.fromUserId)}` : undefined}
              >
    ...
    <div>
      ...
      <div className="mb-6">
        <Note title="How Hub Value works">
          <strong>Hub Value</strong> is a homegrown index ({Math.round(teamPower.weights.winNow * 100)}% win-now +{" "}
          {Math.round(teamPower.weights.futureCapital * 100)}% future capital), scored <strong>relative</strong> to the
          rest of the league on a 0–100 scale. Win-now uses each player&rsquo;s actual production{" "}
          <em>inside our league</em>; future capital uses a synthetic pick-value curve. Players with no league history
          (rookies) are flagged <Tag>unproven</Tag> and get a positional baseline. This is <strong>not</strong> a real
          dynasty trade-market value.
        </Note>
      </div>
        ...
        <div className="space-y-3">
          {teams.map((t, i) => (
            <div key={t.userId} className="flex items-center gap-3">
              <span className="w-5 shrink-0 text-right font-mono text-xs text-[var(--muted)]">{i + 1}</span>
              <div className="w-40 shrink-0 truncate">
                <ManagerChip userId={t.userId} href={`/managers/${t.userId}`} size={20} />
              </div>
              <div className="flex-1">
                <Bar value={t.hubValue} tone="accent" />
              </div>
              <span className="w-12 shrink-0 text-right font-mono text-sm font-semibold tabular-nums text-emerald-300">
                {t.hubValue}
              </span>
