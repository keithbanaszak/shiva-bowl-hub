...
function Bar({ value, tone = "accent" }: { value: number; tone?: "accent" | "accent2" | "muted" }) {
  const color =
    tone === "accent" ? "bg-[var(--accent)]" : tone === "accent2" ? "bg-[var(--accent-2)]" : "bg-[var(--chip)]";
  return (
...
function AxisSlider({ axis }: { axis: number }) {
  ...
  const tone = axis > 20 ? "bg-[var(--accent)]" : axis < -20 ? "bg-[var(--accent-2)]" : "bg-[var(--gold)]";
  return (
    <div className="relative h-2 w-full rounded-full bg-gradient-to-r from-[var(--accent-2-soft)] via-[var(--border)] to-[var(--accent-soft)]">
      <div className="absolute top-1/2 h-1 w-px -translate-y-1/2 bg-[var(--chip)]" style={{ left: "50%" }} />
      <div
    ...
    <Card>
      <div className="mb-3 flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <Link href={`/managers/${t.userId}`} className="block truncate font-display font-semibold hover:text-[var(--accent)]">
            {label(t.userId)}
          </Link>
          ...
          <div key={pos}>
            <div
              className={`mb-1 flex justify-between text-[10px] ${
                pos === t.weakestPos ? "text-[var(--bad)]" : "text-[var(--muted)]"
              }`}
            >
              <span>{pos}</span>
      ...
      <div className="mb-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {t.topAssets.map((p) => (
            <span key={p.playerId} className="flex items-center gap-1.5 rounded-full bg-[var(--card-2)] px-2 py-1 text-xs">
            ...
            t.futurePicks.map((pk, i) => (
              <span
                ...
                className={`rounded px-1.5 py-0.5 text-[10px] ${
                  pk.fromUserId ? "bg-[var(--accent-2-soft)] text-[var(--accent-2)]" : "bg-[var(--card-2)] text-[var(--muted)]"
                }`}
