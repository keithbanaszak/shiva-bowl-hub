  ...
  return (
    <div className="flex items-center gap-2.5 rounded-xl border border-[var(--border)] bg-[var(--panel)] px-2.5 py-1.5">
      ...
      <div className="shrink-0 text-right text-[10px] leading-tight text-[var(--muted)]">
        {e.faab > 0 && <span className="mr-1.5 font-mono text-[var(--gold)]">${e.faab}</span>}
        <span className="whitespace-nowrap">
          {e.season} wk{e.week}
        </span>
        <span className="ml-1.5 hidden whitespace-nowrap text-[var(--faint)] sm:inline">{shortDate(e.dateMs)}</span>
      </div>
    </div>
  );
}
