...
export function TradeFinder() {
  const stances = [...managerStances()].sort((a, b) => b.picks.axis - a.picks.axis);
    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
      {stances.map((s) => {
        ...
        const chips: Array<{ key: string; stance: Stance }> = [
          ...(["QB", "RB", "WR", "TE"] as const).map((p) => ({ key: p, stance: s.positions[p].label })),
            ...
            <div className="mb-3 flex flex-wrap gap-1.5">
              {chips.map((c) => (
                <span
                  ...
                  title={c.stance}
                >
                  {c.key} <span className="text-[9px]">{STANCE_MARK[c.stance]}</span>
                </span>
              ))}
            </div>

            <div className="border-t border-[var(--border)] pt-2">
              <div className="mb-1.5 text-[10px] uppercase tracking-wider text-[var(--muted)]">Best trade fits</div>
              {fits.length === 0 ? (
                <div className="text-xs text-[var(--muted)]">No standout fits right now.</div>
              ) : (
                <ul className="space-y-1.5">
                  {fits.map((f) => (
                    <li key={f.partnerId} className="text-xs">
                      ...
                      <div className="mt-0.5 text-[11px] text-[var(--muted)]">{f.rationale}</div>
                    </li>
                  ))}
                </ul>
              )}
