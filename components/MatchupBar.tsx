...
export function MatchupBar({
  ...
  const body = (
    <div className="flex items-stretch overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--card)] transition group-hover:border-white/25">
      {/* left team (blue) */}
      <div className={`flex flex-1 items-center gap-2 px-3 py-2.5 ${leftTint}`}>
        <Avatar userId={aUserId} size={av} />
        <span className={`min-w-0 flex-1 truncate ${aWon ? "font-semibold text-white" : "text-[var(--muted)]"}`}>
          {label(aUserId)}
        </span>
        <span className="shrink-0 text-right font-mono leading-none">
          <span className={`${scoreCls} tabular-nums ${aWon ? "text-white" : "text-[var(--muted)]"}`}>{aPoints}</span>
      ...
      <div className={`flex flex-1 items-center gap-2 px-3 py-2.5 ${rightTint}`}>
        <span className="shrink-0 text-left font-mono leading-none">
          <span className={`${scoreCls} tabular-nums ${bWon ? "text-white" : "text-[var(--muted)]"}`}>{bPoints}</span>
          {bProj != null && <span className="mt-0.5 block text-[10px] text-[var(--muted)]">{bProj}</span>}
        </span>
        <span className={`min-w-0 flex-1 truncate text-right ${bWon ? "font-semibold text-white" : "text-[var(--muted)]"}`}>
          {label(bUserId)}
        </span>
